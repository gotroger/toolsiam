...
4. ซื้อจริงหนึ่งครั้งตามราคาปัจจุบัน → `/account` ต้องเป็นพรีเมียม · ดู `npx wrangler tail` ว่า webhook ตอบ 200
...
7. ผ่านครบ → สลับ `BEAM_API_BASE_URL` เป็น production + rotate key/secret → ซื้อจริงหนึ่งครั้ง
...
**ราคาต่อเดือนต่ำ + PromptPay ที่ต่ออัตโนมัติไม่ได้** = ผู้ใช้ต้องจำมาสแกนใหม่ทุก 30 วัน
(ปรับเป็น 29 บาท 21 ก.ย. 2569) แรงเสียดทานต่อมูลค่าสูง ทางแก้ที่ถูกที่สุดคือ C3 ขายแพ็กยาว เช่น 1 เดือน 29 · 3 เดือน 79 · 12 เดือน 269
...
**กระทบธุรกิจ:** จุดคุ้มทุนขยับจาก ~7 คน เป็น ~24 คน (Workers 180 + VPS 500 = 680 ÷ 29)
