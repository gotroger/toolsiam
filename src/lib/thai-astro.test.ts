...
describe('ราศีสากล', () => {
  ...
  it('ธาตุ คุณภาพ และดาวเจ้าเรือนตรงตามโหราศาสตร์สากล', () => {
    // สำเนาอิสระ ไม่ import จากซอร์ส: ลำดับ ไฟ ดิน ลม น้ำ และ จร สถิร อุภัย วนตามจักรราศี
    const elements = ['ไฟ', 'ดิน', 'ลม', 'น้ำ'];
    const qualities = ['จรราศี', 'สถิรราศี', 'อุภัยราศี'];
    const planets = [
      'อังคาร',
      'ศุกร์',
      'พุธ',
    ...
    ZODIAC_SIGNS.forEach((s, i) => {
      expect(s.element, s.id).toBe(elements[i % 4]);
      expect(s.quality, s.id).toBe(qualities[i % 3]);
      expect(s.planet, s.id).toContain(planets[i]);
    });
  });

  it('ราศีที่เข้ากัน: ธาตุเดียวกัน 2 ราศี ธาตุคู่ส่งเสริม 3 ราศี และไม่รวมตัวเอง', () => {
    for (const s of ZODIAC_SIGNS) {
      const c = compatibleSigns(s);
      expect(c.sameElement.length, s.id).toBe(2);
      expect(c.complementary.length, s.id).toBe(3);
      expect(c.sameElement.some((x) => x.id === s.id)).toBe(false);
    }
    expect(compatibleSigns(zodiacById('aries')!).sameElement.map((x) => x.id)).toEqual(['leo', 'sagittarius']);
    expect(compatibleSigns(zodiacById('aries')!).complementary.map((x) => x.id)).toEqual([
      'gemini',
      'libra',
      'aquarius',
    ]);
  ...
  it('ช่วงวันและสัญลักษณ์แบบตัวอักษร', () => {
    expect(zodiacRange(zodiacById('aries')!)).toBe('21 มี.ค. – 19 เม.ย.');
    expect(zodiacGlyph(zodiacById('aries')!)).toBe('♈\uFE0E');
  });
