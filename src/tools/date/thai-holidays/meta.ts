import type { ToolMeta } from '@/tools/types';

export const thaiHolidaysMeta: ToolMeta = {
  slug: 'thai-holidays',
  name: 'วันหยุดราชการและวันหยุดธนาคาร 2569 + นับวันทำการ',
  nameEn: 'Thai Public & Bank Holidays 2026 / Business Day Counter',
  category: 'date',
  featuredRank: 40,
  description:
    'ดูวันหยุดราชการและวันหยุดธนาคารปี 2569 ครบทั้งปีในที่เดียว พร้อมนับจำนวนวันทำการระหว่างสองวันที่ และคำนวณว่าอีกกี่วันทำการจะถึงวันไหน เลือกได้ว่าจะใช้ปฏิทินราชการหรือปฏิทินธนาคาร',
  keywords: ['วันหยุด 2569', 'วันหยุดธนาคาร 2569', 'วันหยุดราชการ 2569', 'นับวันทำการ', 'ปฏิทินวันหยุดปีหน้า'],
  howTo: [
    'เลือกปฏิทินที่ต้องการ: วันหยุดธนาคาร (ตามประกาศ ธปท.) หรือวันหยุดราชการ',
    'ดูรายการวันหยุดทั้งปีพร้อมวันในสัปดาห์ หรือเลื่อนลงไปที่เครื่องมือนับวันทำการ',
    'กรอกวันเริ่มและวันสิ้นสุดเพื่อนับวันทำการ หรือกรอกจำนวนวันทำการเพื่อหาว่าครบกำหนดวันไหน',
  ],
  faq: [
    {
      q: 'วันหยุดธนาคารกับวันหยุดราชการต่างกันอย่างไร',
      a: 'ไม่เหมือนกันทุกวัน เช่น วันแรงงานแห่งชาติ 1 พฤษภาคม ธนาคารหยุดแต่ราชการเปิด ส่วนวันพืชมงคลและวันเข้าพรรษา ราชการหยุดแต่ธนาคารเปิดทำการตามปกติ',
    },
    {
      q: 'ปี 2569 ธนาคารหยุดกี่วัน',
      a: 'ตามประกาศธนาคารแห่งประเทศไทย วันหยุดของสถาบันการเงินปี 2569 ที่ตรงกับวันทำการทั่วประเทศมี 19 วัน และกรุงเทพมหานครมี 20 วัน โดยรวมวันหยุดพิเศษวันที่ 2 มกราคม และวันหยุดชดเชยแล้ว',
    },
    {
      q: 'รวมวันหยุดพิเศษ 16 ตุลาคม 2569 หรือยัง',
      a: 'เลือกพื้นที่กรุงเทพมหานครเพื่อรวมวันหยุดพิเศษนี้ทั้งปฏิทินราชการและธนาคาร; ตัวเลือกทั่วประเทศไม่รวมวันหยุดเฉพาะพื้นที่; วันที่ 12, 14 และ 15 ตุลาคมที่กำหนดให้ WFH ยังคงเป็นวันทำการ',
    },
    {
      q: 'นับวันทำการข้ามไปปี 2570 ได้ไหม',
      a: 'ได้ แต่ขณะนี้ยังไม่มีประกาศวันหยุดปี 2570 อย่างเป็นทางการ ส่วนที่เลยปี 2569 ไปจึงนับเฉพาะเสาร์–อาทิตย์เป็นวันหยุด และเครื่องมือจะขึ้นคำเตือนทุกครั้ง เมื่อมีประกาศ ธปท. และมติ ครม. แล้วจะเพิ่มวันหยุดของปีนั้นให้',
    },
    {
      q: 'นับวันทำการรวมวันเริ่มต้นด้วยไหม',
      a: 'ตัวนับช่วงวันที่นับรวมทั้งวันเริ่มต้นและวันสิ้นสุด ส่วนโหมดบวกวันทำการจะนับจากวันถัดจากวันที่กรอก ซึ่งตรงกับวิธีนับเครดิตเทอมและกำหนดส่งเอกสารส่วนใหญ่',
    },
  ],
};
