...
describe('ตำราทำนายฝัน', () => {
  ...
  // เลขนำโชคมีได้เฉพาะใน lucky-numbers.ts (ตัดสินใจ 20 ก.ย. 2569) — เนื้อความยังห้ามชวนเสี่ยงโชคเหมือนเดิม
  it('เนื้อความไม่ชวนเสี่ยงโชค — เป็นข้อจำกัดของเว็บ ไม่ใช่แค่สไตล์', () => {
    const banned = /เลขเด็ด|ซื้อหวย|แทงหวย|เลขท้าย|งวดนี้|ถูกรางวัล/;
    for (const e of DREAM_ENTRIES) {
